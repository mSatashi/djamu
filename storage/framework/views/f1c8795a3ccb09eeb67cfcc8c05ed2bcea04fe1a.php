    ...
    <input type="button" value="+" id="plus" onclick="plus()">
					...
					<div class="col-md-8">
						...
						<div class="form-group ">
							...
							<input type="hidden" value="2" class="form-control" name="province_origin">
						</div>
						<div class="form-group ">
							<label>Kota Asal</label>
							<input type="hidden" value="27" class="form-control" id="city_origin" name="city_origin">
						</div>
						<div class="form-group ">
							<label>
								Alamat<span>*</span>
						...
						<div class="form-group form-group--inline">
							...
							<select name="province_id" id="province_id" class="form-control">
								...
								<?php $__currentLoopData = $provinsi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
									<option value="<?php echo e($row['province_id']); ?>" namaprovinsi="<?php echo e($row['province']); ?>"><?php echo e($row['province']); ?></option>
						...
						<div class="form-group ">
							...
							<input type="text" name="totalbelanja" class="form-control" value="20000" readonly="on">
						</div>
						<div class="form-group ">
							<label>total berat (gram) </label>
							<input class="form-control" type="text" value="200" id="weight" name="weight" readonly="on">
						</div>
						<div class="form-group ">
							<label>total ongkos kirim </label>
							<input class="form-control" type="text" id="ongkos_kirim" name="ongkos_kirim" readonly="on">
						</div>
						<div class="form-group ">
							<label>total keseluruhan </label>
							<input class="form-control" type="text" id="total_keseluruhan" name="total_keseluruhan" readonly="on">
						</div>
						<div class="form-group">
		...
		<script>
			$(document).ready(function(){
					...
					// jika di temukan id nya kita buat eksekusi ajax GET
						...
						// url yg di root yang kita buat tadi
							...
							success:function(data){
								...
								$('select[name="kota_id"]').append('<option value="" namakota="">Pilih Salah Satu</option>');
					...
					if(courier){
						jQuery.ajax({
							...
							success:function(data){
								...
								$('select[name="layanan"]').append('<option value="" ongkir="">Pilih Jenis Layanan</option>');
								...
								$.each(data, function(key, value){
									...
									$.each(value.costs, function(key1, value1){
										...
										$.each(value1.cost, function(key2, value2){
											$('select[name="layanan"]').append('<option value="'+ key +'" ongkir="' + value2.value+ '">' + value1.service + '-' + value1.description + '-' +value2.value+ '</option>');
										});
									});
								});
							}
				...
				//memberikan action ketika select name layanan di select
				$('select[name="layanan"]').on('change', function(){
					// membuat variable ongkirku untuk mendapatkan atribut ongkir
					var ongkirku = $("#layanan option:selected").attr("ongkir");
					// menampilkan hasil ongkir ke input id ongkos_kirim
					$("#ongkos_kirim").val(ongkirku);
				});

				//memberikan action kepada total belanjaan ketika select name layanan di select
				$('select[name="layanan"]').on('change', function(){
					// membuat variable ongkirku untuk mendapatkan atribut ongkir
					var ongkirku = $("#layanan option:selected").attr("ongkir");
					// name total harga di dapat dari inputan harga
					let totalbelanjaku = $("input[name=totalbelanja]").val();
					$("#total_keseluruhan").val(parseInt(ongkirku)+parseInt(totalbelanjaku));
				});
			});
		</script>
		<script>
...
</html><?php /**PATH F:\d'jamu\resources\views/checkout.blade.php ENDPATH**/ ?>